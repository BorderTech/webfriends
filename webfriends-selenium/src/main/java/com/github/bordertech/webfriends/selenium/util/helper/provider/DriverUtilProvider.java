...
/**
 * Driver utility provider.
 */
public interface DriverUtilProvider {
	 ...
	 */
	public default FriendDriver createDriverInstance(final WebDriverType driverType) {
		if (driverType == null) {
	 ...
	 * @return a FriendDriver wrapping the given backing driver
	 */
	public default FriendDriver createDriverInstance(final WebDriver webDriver) {
		if (webDriver == null) {
			throw new IllegalArgumentException("Backing driver must be provided.");
		}
		FriendDriver friend = Factory.newInstance(FriendDriver.class, FriendDriver.class);
	 ...
	 */
	public default void configDriver(final WebDriver driver) {
		if (driver == null) {
			throw new IllegalArgumentException("A web driver must be provided.");
		}
		configWait(driver);
		int width = ConfigurationProperties.getScreenWidth();
		int height = ConfigurationProperties.getScreenHeight();
	 ...
	 * @param driver the web driver to configure.
	 */
	public default void configWait(final WebDriver driver) {
		long seconds = ConfigurationProperties.getImplicitWait();
	 ...
	 */
	public default void configNoWait(final WebDriver driver) {
	 ...
	 */
	public default void setImplicitWait(final WebDriver driver, final long time, final TimeUnit unit) {
		if (driver == null) {
			throw new IllegalArgumentException("A web driver must be provided.");
		}
	 ...
	 */
	public default void waitForAjaxReady(final WebDriver driver) {
	 ...
	 * @param driver the web driver
	 */
	public default void waitForPageReady(final WebDriver driver) {
		Function<WebDriver, Boolean> condition = getPageReadyCondition();
		if (condition != null) {
			int timeout = ConfigurationProperties.getPageReadyTimeout();
			long polling = ConfigurationProperties.getPageReadyPollInterval();
	 ...
	 */
	public default void verifyPageLoaded(final WebDriver driver, final Function<WebDriver, Boolean> condition) {
		// Check page is ready before verifying page
		waitForPageReady(driver);
		if (condition != null) {
			int timeout = ConfigurationProperties.getVerifyPageTimeout();
			long polling = ConfigurationProperties.getVerifyPagePollInterval();
			waitForCondition(driver, timeout, polling, condition);
		}
	}
	 ...
	 */
	public default void waitForCondition(final WebDriver driver, final int timeoutSeconds, final long pollingMilliseconds, final Function<WebDriver, Boolean> condition) {
		if (driver == null) {
			throw new IllegalArgumentException("A web driver must be provided.");
		}
		if (condition == null) {
	 ...
	 */
	public default PageReadyCondition getPageReadyCondition() {
	 ...
	 */
	public default void setFocus(final FriendDriver driver, final SElement element) {
	 ...
	 */
	public default void setFocus(final FriendDriver driver, final WebElement webElement) {
		// Get element ID
