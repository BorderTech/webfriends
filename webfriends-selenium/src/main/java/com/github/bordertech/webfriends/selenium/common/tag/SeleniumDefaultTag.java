...
public class SeleniumDefaultTag<T extends SElement> implements SeleniumTag<T> {
	...
	public SeleniumDefaultTag(final Class<T> tagClass, final ElementTag backing) {
		...
		this.xpathRoot = Helper.getProvider().buildElementQualifierXpath(backing.getTagName(), backing.getQualifiers(), false);
		this.xpathRelative = Helper.getProvider().buildElementQualifierXpath(backing.getTagName(), backing.getQualifiers(), true);
	}
